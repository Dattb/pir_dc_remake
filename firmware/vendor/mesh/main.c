...
_attribute_ram_code_ int main (void)    //must run in ramcode
{
...
#endif
	unsigned char joinState = flashReadJoinFlag();

	#if(SUART_DEBUG)
		sUartInit(&sUart1);
	#endif
	rd_factory_reset(8,btnStateRead,joinState);
	sUartInit(&sUart1);

	if(joinState == 0x00){
		if(!resetFlag){
			pirDCInit(&rdPir);
			pirMotionCOntrol(&rdPir.flag,45);
			pirDetect(&rdPir);
			sendLuxFlag = LuxDetect(30);
		}
		if (!rdOta.otaFlag)
			sleepBeforSend(rdPir.flag.motionFlag,rdPir.flag.motionFlagOld ,sendLuxFlag);
	}
	else{
		#if(SUART_DEBUG)
			rdPrintf("unprov\n");
	...
	if(is_provision_success()){
		if(joinState == 0x00){
			...
			#endif
			if(!resetFlag)
				sensorSendData(rdPir.flag.motionFlag,rdPir.flag.motionFlagOld,sendLuxFlag);
			if (!rdOta.otaFlag)
				sleepAfterSend();
		}
		else {
	...
	while (1){
		if(!rdOta.otaFlag)
			...
			wd_clear(); //clear watch dog
		#endif
		if(!rdOta.otaFlag && (joinState == 0 || (confirmDone && (clock_time_ms() - endConfigTime >= 1000)))){
			#if(SUART_DEBUG)
				rdPrintf("sleep_3\n");
			#endif
			sleepAfterSend();
