...
void RD_light_prov_success_with_sleep(u32 count, u32 half_cycle_us)
{
	gpio_set_func(GPIO_PB6, AS_GPIO);
	gpio_set_output_en(GPIO_PB6, 1);
	gpio_set_input_en(GPIO_PB6, 0);
...
void RD_light_ota_with_sleep(u32 count, u32 half_cycle_us)
{
	gpio_set_func(GPIO_PB5, AS_GPIO);
	gpio_set_output_en(GPIO_PB5, 1);
	gpio_set_input_en(GPIO_PB5, 0);
...
_USER_CAN_REDEFINE_ void show_ota_result(int result)
{
	if(result == OTA_SUCCESS){
		light_ev_with_sleep(6, 500*1000);	//0.5Hz shine for  6 second
	}
	else{
...
{
	//light_ev_with_sleep(5, 500*1000);	//1Hz shine for  8 second
	RD_light_ev_with_sleep(10, 100*1000);	//1Hz shine for  2 second
}
#endif
